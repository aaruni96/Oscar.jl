...
julia> rays_modulo_lineality(maximal_groebner_cone(G,ord))
(rays_modulo_lineality = RayVector{QQFieldElem}[[1, 0, 0], [0, 1, 1], [0, 1, 0]], lineality_basis = RayVector{QQFieldElem}[])

julia> w = homogeneity_vector(G)
3-element Vector{ZZRingElem}:
 1
 1
 1

julia> rays_modulo_lineality(maximal_groebner_cone(G,ord,w))
(rays_modulo_lineality = RayVector{QQFieldElem}[[0, 0, -1]], lineality_basis = RayVector{QQFieldElem}[[1, 0, 0], [0, 1, 1]])

```
"""
function maximal_groebner_cone(G::Vector{<:MPolyElem}, ord::MonomialOrdering)
    # calling with no homogeneityWeight
    # is calling with `nothing` as homogeneityWeight
    return maximal_groebner_cone(G,ord,nothing)
end
...
function polyhedral_fan_from_cones(listOfCones::Vector{<:Cone})
    ...
    return PolyhedralFan(fanRays,fanLineality,fanIncidences)
end
